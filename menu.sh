...
# Repository: https://github.com/mkkelati/script11
...
CLOUDFLARE_CONFIG_DIR="/etc/mk-script/cloudflare"
CLOUDFLARE_CONFIG_FILE="$CLOUDFLARE_CONFIG_DIR/tunnel.conf"
...
print_menu() {
    ...
    echo -e "${RED}[${BLUE}06${RED}] ${WHITE}◇ ${BLUE}Network Traffic${RESET}      ${RED}[${BLUE}13${RED}] ${WHITE}◇ ${CYAN}Cloudflare Tunnel${RESET}"
...
detect_current_setup() {
    echo -e "${YELLOW}🔍 Detecting current server configuration...${RESET}"
    
    # Check stunnel status
    local stunnel_status="INACTIVE"
    local stunnel_port="443"
    
    if systemctl is-active --quiet stunnel4 2>/dev/null; then
        stunnel_status="ACTIVE"
        stunnel_port=$(grep -m1 "^accept = " /etc/stunnel/stunnel.conf 2>/dev/null | awk '{print $3}' || echo "443")
    fi
    
    # Get server info
    local server_ip=$(curl -s --connect-timeout 5 ifconfig.me 2>/dev/null || echo "Unable to detect")
    local ubuntu_version=$(lsb_release -rs 2>/dev/null || echo "Unknown")
    local user_count=$(wc -l < "$USER_LIST_FILE" 2>/dev/null || echo "0")
    
    echo -e "\n${GREEN}✅ Current Setup Detected:${RESET}"
    echo -e "${WHITE}   • Server IP:${RESET} ${YELLOW}$server_ip${RESET}"
    echo -e "${WHITE}   • Ubuntu Version:${RESET} ${YELLOW}$ubuntu_version${RESET}"
    echo -e "${WHITE}   • Stunnel Status:${RESET} ${YELLOW}$stunnel_status${RESET} (Port: ${YELLOW}$stunnel_port${RESET})"
    echo -e "${WHITE}   • Active Users:${RESET} ${YELLOW}$user_count${RESET}"
    
    return 0
}

install_cloudflared() {
    echo -e "${YELLOW}📦 Installing Cloudflare Tunnel daemon...${RESET}"
    
    # Check if already installed
    if [[ -f "$CLOUDFLARE_BINARY" ]] && command -v cloudflared >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Cloudflared already installed${RESET}"
        return 0
    fi
    
    # Create temporary directory
    local temp_dir="/tmp/cloudflared-install"
    mkdir -p "$temp_dir"
    cd "$temp_dir"
    
    # Download latest cloudflared
    echo -e "${YELLOW}⬇️  Downloading latest cloudflared...${RESET}"
    if wget -q --show-progress "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64" -O cloudflared; then
    ...
    # Install binary
    chmod +x cloudflared
    mv cloudflared "$CLOUDFLARE_BINARY"
    ln -sf "$CLOUDFLARE_BINARY" /usr/local/bin/cloudflared 2>/dev/null
    
    # Verify installation
    if command -v cloudflared >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Cloudflared installed successfully${RESET}"
        echo -e "${WHITE}   Version: ${YELLOW}$(cloudflared version 2>/dev/null | head -1)${RESET}"
        return 0
    else
...
configure_cloudflare_tunnel() {
    echo -e "${CYAN}🌐 Cloudflare Tunnel Configuration${RESET}"
    echo -e "${WHITE}Please provide your Cloudflare Tunnel information:${RESET}\n"
    
    # Get domain name
    echo -e "${YELLOW}📋 Step 1: Domain Configuration${RESET}"
    while true; do
        read -p "Enter your domain name (e.g., mknew1.gzz.io): " domain_name
        if [[ -n "$domain_name" ]] && [[ "$domain_name" =~ ^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$ ]]; then
            break
        else
            echo -e "${RED}❌ Invalid domain format. Please try again.${RESET}"
        fi
    done
    
    # Get tunnel token
    echo -e "\n${YELLOW}🔑 Step 2: Tunnel Token${RESET}"
    echo -e "${WHITE}Paste your Cloudflare Tunnel token (from Cloudflare dashboard):${RESET}"
    while true; do
        read -s tunnel_token
        if [[ -n "$tunnel_token" ]] && [[ ${#tunnel_token} -gt 50 ]]; then
            break
        else
            echo -e "${RED}❌ Invalid token format. Please paste the complete token.${RESET}"
        fi
    done
    
    echo -e "\n${YELLOW}🔧 Configuring tunnel...${RESET}"
...
# Cloudflare Tunnel Configuration
# Generated by MK Script Manager v4.0
...
EOF
    ...
    # Create systemd service
    create_cloudflare_service "$tunnel_token"
...
ExecStart=$CLOUDFLARE_BINARY tunnel --no-autoupdate run --token $token
Restart=on-failure
RestartSec=5s
...
start_cloudflare_tunnel() {
    ...
    if systemctl start cloudflared; then
        ...
        if systemctl is-active --quiet cloudflared; then
            echo -e "${GREEN}✅ Cloudflare Tunnel started successfully!${RESET}"
            return 0
        else
        ...
        echo -e "${RED}❌ Failed to start tunnel service${RESET}"
        return 1
    fi
}

stop_cloudflare_tunnel() {
    echo -e "${YELLOW}🛑 Stopping Cloudflare Tunnel...${RESET}"
    
    if systemctl stop cloudflared; then
        echo -e "${GREEN}✅ Cloudflare Tunnel stopped${RESET}"
        return 0
    else
...
check_cloudflare_status() {
    local status_text="${RED}INACTIVE ○${RESET}"
    ...
    echo -e "${WHITE}Tunnel Status: $status_text${RESET}"
    echo -e "${WHITE}Domain: ${YELLOW}$domain${RESET}"
...
generate_http_injector_configs() {
    if [[ ! -f "$CLOUDFLARE_CONFIG_FILE" ]]; then
        echo -e "${RED}❌ Cloudflare Tunnel not configured yet${RESET}"
        return 1
    fi
    ...
    clear
    display_header_with_timestamp "HTTP INJECTOR CONFIGS"
    
    echo -e "\n${GREEN}📱 HTTP Injector Configuration Templates${RESET}"
    echo -e "${BLUE}════════════════════════════════════════${RESET}\n"
    
    echo -e "${YELLOW}🔧 Standard Configuration:${RESET}"
    echo -e "${WHITE}Server Host: ${GREEN}$DOMAIN_NAME${RESET}"
    echo -e "${WHITE}Server Port: ${GREEN}443${RESET}"
    echo -e "${WHITE}Protocol: ${GREEN}SSL/TLS${RESET}"
    echo -e "${WHITE}SSL/SNI: ${GREEN}$DOMAIN_NAME${RESET}"
    
    echo -e "\n${YELLOW}🎭 SNI Spoofing Options:${RESET}"
    echo -e "${WHITE}Server Host: ${GREEN}$DOMAIN_NAME${RESET}"
    echo -e "${WHITE}Server Port: ${GREEN}443${RESET}"
    echo -e "${WHITE}Protocol: ${GREEN}SSL/TLS${RESET}"
    echo -e "${WHITE}SSL/SNI: ${GREEN}google.com${RESET} (or facebook.com, youtube.com)"
    
    echo -e "\n${CYAN}💡 Connection Notes:${RESET}"
    echo -e "${WHITE}• Use SSL/TLS encryption: ${GREEN}ENABLED${RESET}"
    echo -e "${WHITE}• Connection will look like normal HTTPS traffic${RESET}"
    echo -e "${WHITE}• ISP will see connection to Cloudflare (trusted)${RESET}"
    echo -e "${WHITE}• Your real server IP is hidden${RESET}"
...
uninstall_cloudflare_tunnel() {
    ...
    read -p "Are you sure you want to remove Cloudflare Tunnel? (y/n): " confirm
    if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
    ...
    # Remove files
    rm -f "$CLOUDFLARE_SERVICE_FILE"
    rm -f "$CLOUDFLARE_BINARY"
    rm -rf "$CLOUDFLARE_CONFIG_DIR"
    rm -f /usr/local/bin/cloudflared
    
    # Reload systemd
    systemctl daemon-reload
    
    echo -e "${GREEN}✅ Cloudflare Tunnel uninstalled successfully${RESET}"
}

manage_cloudflare_tunnel() {
    while true; do
        clear
        display_header_with_timestamp "CLOUDFLARE TUNNEL MANAGER"
        
        echo -e "\n${BLUE}┌────────────────────────────────────────────────────────────┐${RESET}"
        echo -e "${BLUE}│${WHITE}                  CLOUDFLARE TUNNEL MANAGER                  ${BLUE}│${RESET}"
        echo -e "${BLUE}└────────────────────────────────────────────────────────────┘${RESET}\n"
        ...
        # Menu options
        echo -e "${RED}[${BLUE}1${RED}] ${WHITE}• ${GREEN}Install Cloudflare Tunnel${RESET}"
        echo -e "${RED}[${BLUE}2${RED}] ${WHITE}• ${CYAN}Configure Tunnel (Domain + Token)${RESET}"
        echo -e "${RED}[${BLUE}3${RED}] ${WHITE}• ${YELLOW}Start Tunnel Service${RESET}"
        echo -e "${RED}[${BLUE}4${RED}] ${WHITE}• ${RED}Stop Tunnel Service${RESET}"
        ...
        case "$choice" in
            ...
            2)
                if ! command -v cloudflared >/dev/null 2>&1; then
                    echo -e "${RED}❌ Please install Cloudflare Tunnel first (option 1)${RESET}"
                else
                    configure_cloudflare_tunnel
                fi
                ;;
            3)
                if [[ ! -f "$CLOUDFLARE_CONFIG_FILE" ]]; then
                    echo -e "${RED}❌ Please configure tunnel first (option 2)${RESET}"
                else
                    start_cloudflare_tunnel
                fi
                ;;
            4)
                ...
                ;;
            5)
                echo -e "\n${CYAN}📊 Detailed Tunnel Status:${RESET}"
                check_cloudflare_status
                if systemctl is-active --quiet cloudflared; then
                    echo -e "\n${GREEN}🔄 Service Logs (last 10 lines):${RESET}"
                    journalctl -u cloudflared --no-pager -n 10 --since "1 hour ago" 2>/dev/null || echo "No logs available"
                fi
                ;;
            6)
                generate_http_injector_configs
